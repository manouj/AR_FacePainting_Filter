...
public class TexturePainter : MonoBehaviour {
    ...
    public GameObject prevBrushObj;
	...
	void Update () {
		...
		UpdateBrushCursor ();
        if (brushes == null)
            brushes = GameObject.FindGameObjectsWithTag("BrushEntity(Clone)");
        if (!Input.GetMouseButton(0))
        {
            prevBrushObj = null;
        }
	...
	void DoAction(){	
		...
		if(HitTestUVPosition(ref uvWorldPosition)){
			...
			if(mode==Painter_BrushMode.PAINT){
                ...
                brushObj.GetComponent<lineRenderer>().origin = brushObj.transform;
                if (Input.GetMouseButton(0))
                { 
                    if (prevBrushObj!= null)
                    {
                    brushObj.GetComponent<lineRenderer>().destination = prevBrushObj.transform;
                }
                prevBrushObj = brushObj;
                }
				...
				brushObj=(GameObject)Instantiate(Resources.Load("TexturePainter-Instances/DecalEntity1")); //Paint a decal
			}
